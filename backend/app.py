...
DB_CFG=dict(host=os.getenv("POSTGRES_HOST","db"),port=int(os.getenv("POSTGRES_PORT","5432")),dbname=os.getenv("POSTGRES_DB","nasa2025"),user=os.getenv("POSTGRES_USER","nasa2025"),password=os.getenv("POSTGRES_PASSWORD","nasa2025"))
db_pool=None
...
def preload_cities():
    ...
    try:
        r=requests.get("https://public.opendatasoft.com/api/explore/v2.1/catalog/datasets/worldcitiespop/records?limit=50000",timeout=30)
        ...
        for row in rows:
            ...
            if not(code and name and lat and lon):continue
            if code not in tmp:tmp[code]={"country":code,"cities":[]}
            if len(tmp[code]["cities"])<100:
                tmp[code]["cities"].append({"name":name.title(),"lat":lat,"lon":lon})
        STATIC_CITIES=tmp
        os.makedirs("/app/static/data",exist_ok=True)
        with open("/app/static/data/static_cities.json","w") as f:json.dump(STATIC_CITIES,f)
        app.logger.info(f"[City preload] {len(STATIC_CITIES)} countries cached")
    except Exception as e:
        app.logger.warning(f"[City preload fail] {e}")
        if os.path.exists("/app/static/data/static_cities.json"):
            with open("/app/static/data/static_cities.json") as f:STATIC_CITIES=json.load(f)
...
def init_db_pool():
    ...
    while time.time()<deadline:
        try:
            ...
            with conn,conn.cursor() as cur:cur.execute("SELECT 1;")
            db_pool.putconn(conn);break
        except Exception as e:
            last_err=e;app.logger.warning(f"[DB wait] {e}");time.sleep(2)
    if not db_pool:raise RuntimeError(f"Database not reachable: {last_err}")
    ensure_schema()
    ...
    try:
        with conn:
            with conn.cursor() as cur:
                ...
                cur.execute("""CREATE TABLE IF NOT EXISTS app_users(id BIGINT GENERATED ALWAYS AS IDENTITY PRIMARY KEY,username TEXT UNIQUE NOT NULL,pin_hash TEXT NOT NULL,created_at TIMESTAMPTZ DEFAULT NOW());""")
                cur.execute("""CREATE TABLE IF NOT EXISTS events(id BIGINT GENERATED ALWAYS AS IDENTITY PRIMARY KEY,user_id BIGINT NOT NULL REFERENCES app_users(id) ON DELETE CASCADE,event_name TEXT NOT NULL,date DATE NOT NULL,city TEXT,country TEXT,lat DOUBLE PRECISION,lon DOUBLE PRECISION,created_at TIMESTAMPTZ DEFAULT NOW());""")
...
def _hash_pin(username,pin):return hashlib.sha256(f"{username}:{pin}:{APP_SECRET}".encode()).hexdigest()
def _sign(payload):data=json.dumps(payload,separators=(",",":"));sig=hashlib.sha256((data+APP_SECRET).encode()).hexdigest();return f"{data}.{sig}"
def _verify(token):
    try:
...
def issue_token(uid,username):return _sign({"uid":uid,"username":username,"exp":time.time()+60*60*24*7})
def require_auth(fn):
    @wraps(fn)
    def wrapper(*a,**kw):
...
def get_meteomatics_summary(lat,lon,date_iso):
    ...
    try:
        ...
        js=r.json();data={p["parameter"]:p["coordinates"][0]["dates"][0]["value"]for p in js.get("data",[])if p.get("coordinates")}
...
def get_nasa_power(lat,lon,date_iso):
    try:
        ...
        return{"tmax":tmax,"tmin":tmin,"avg_temp":(tmax+tmin)/2.0,"precip":precip}
    except:return None

def interpret_conditions(m,event_name):
    if not m:return"mixed conditions",["bring umbrella just in case","water bottle","sunscreen"]
    tips=[];tmax=m.get("t_max");rain=m.get("precip_24h",0)or 0
    ...
    if any(k in e for k in["drone","flying","aerial"]):tips+=["check wind before flight","spare batteries"]
    elif any(k in e for k in["wedding","ceremony","party"]):tips+=["backup indoor location"]
    elif any(k in e for k in["picnic","bbq","beach","park"]):tips+=["sunscreen","ground mat"]
    elif any(k in e for k in["hike","trail","trek"]):tips+=["hydration pack","insect repellent"]
...
def signup():
    ...
    ph=_hash_pin(u,p);conn=db_pool.getconn()
    try:
        with conn,conn.cursor(cursor_factory=RealDictCursor)as c:
            ...
            row=c.fetchone()
            if not row:
                c.execute("SELECT id,pin_hash FROM app_users WHERE username=%s;",(u,));row2=c.fetchone()
...
def login():
    ...
    try:
        with conn,conn.cursor(cursor_factory=RealDictCursor)as c:
            c.execute("SELECT id,pin_hash FROM app_users WHERE username=%s;",(u,));row=c.fetchone()
...
def suggest(user):
    ...
    event_name=(d.get("event_name")or"").strip();date=d.get("date");lat=d.get("lat");lon=d.get("lon");event_id=d.get("event_id")
    if event_id and(not date or lat is None or lon is None or not event_name):
        ...
        try:
            with conn,conn.cursor(cursor_factory=RealDictCursor)as c:
                ...
                event_name=event_name or row["event_name"];date=date or row["date"];lat=lat if lat is not None else row["lat"];lon=lon if lon is not None else row["lon"]
        finally:db_pool.putconn(conn)
    mm=nasa=None
    if date and lat and lon:mm=get_meteomatics_summary(float(lat),float(lon),date);nasa=get_nasa_power(float(lat),float(lon),date)
    label,tips=interpret_conditions(mm,event_name);stats_text=None
    if mm:
        tmax=mm.get("t_max");rain=mm.get("precip_24h",0)
        if tmax is not None:stats_text=f"(Max {tmax:.1f}°C, Rain {rain:.1f} mm)"
    elif nasa:stats_text=f"(Max {nasa.get('tmax',0):.1f}°C, Rain {nasa.get('precip',0):.1f} mm)"
...
def geo_countries():
    try:
        now=time.time()
        if COUNTRY_CACHE["data"] and now-COUNTRY_CACHE["ts"]<CACHE_TTL_COUNTRIES:return jsonify(COUNTRY_CACHE["data"])
        r=requests.get("https://restcountries.com/v3.1/all?fields=name,cca2",timeout=15);r.raise_for_status()
        out=[{"name":(it.get("name",{})or{}).get("common"),"code":it.get("cca2","").upper()}for it in r.json()if(it.get("name",{})or{}).get("common")and it.get("cca2")]
        out.sort(key=lambda x:x["name"]);COUNTRY_CACHE["data"]=out;COUNTRY_CACHE["ts"]=now;return jsonify(out)
    except:return jsonify([])
@app.get("/geo/cities")
def geo_cities():
    code=(request.args.get("code")or"").strip().upper()
    if not code:return jsonify({"cities":[]})
    data=STATIC_CITIES.get(code)
    if not data:return jsonify({"code":code,"country":code,"cities":[]})
    return jsonify({"code":code,"country":data.get("country",code),"cities":data.get("cities",[])})
...
def api_reverse_geocode():
    ...
    try:
        ...
        country=addr.get("country");code=addr.get("country_code").upper()if addr.get("country_code")else None
        return jsonify({"city":city,"country":country,"country_code":code})
    except:return jsonify({})
@app.get("/")
